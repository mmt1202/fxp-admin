...
import { ImportTask, ImportTaskError, ImportType, importTaskApi, importTypeText } from '../api/importTasks';
    ...
    <div className="import-page">
      ...
      <section className="panel-card">
        ...
        <form className="filter-bar" onSubmit={handleSubmit}>
          <label>导入类型<select value={type} onChange={(event) => setType(event.target.value as ImportType)}>{Object.entries(importTypeText).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
          <label>上传文件<input type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={handleFileChange} /></label>
          <button type="submit" disabled={submitting}>{submitting ? '提交中...' : '提交导入'}</button>
        </form>
        <p className="helper-text">必填字段：{expectedFields.join('、')}。提交前会预览前 20 行并阻断明显字段错误。</p>
      </section>

      <section className="panel-card">
        <h2>预览前 20 行</h2>
        <div className="table-wrap"><table><thead><tr><th>行号</th><th>状态</th><th>数据</th><th>失败原因</th></tr></thead><tbody>{previewRows?.length ? previewRows.map((row) => <tr key={row.rowNumber}><td>{row.rowNumber}</td><td><span className={`status-pill ${row.valid ? 'success' : 'muted'}`}>{row.valid ? '通过' : '失败'}</span></td><td>{Object.entries(row.values).map(([key, value]) => `${key}: ${value}`).join('；')}</td><td>{row.errors.join('；') || '-'}</td></tr>) : <tr><td className="empty-cell" colSpan={4}>请选择 CSV 文件后查看预览；XLSX 预览由后端返回。</td></tr>}</tbody></table></div>
      </section>

      <section className="panel-card">
        <h2>导入任务</h2>
        <div className="table-wrap"><table><thead><tr><th>任务</th><th>类型</th><th>状态</th><th>成功/失败/总数</th><th>创建时间</th><th>操作</th></tr></thead><tbody>{tasks.map((task) => <tr key={task.id}><td>{task.fileName}<br /><small>{task.id}</small></td><td>{importTypeText[task.type]}</td><td><span className="status-pill muted">{task.status}</span></td><td>{task.successRows} / {task.failedRows} / {task.totalRows}</td><td>{task.createdAt}</td><td><div className="action-group"><button type="button" className="secondary-button" onClick={() => handleShowErrors(task)}>失败原因</button>{task.errorFileUrl ? <a className="secondary-button" href={task.errorFileUrl}>下载错误文件</a> : null}</div></td></tr>)}</tbody></table></div>
      </section>

      {selectedTask && <section className="detail-panel"><div><h2>{selectedTask.fileName} 失败原因</h2><p>来自接口 GET /admin/import/tasks/:id/errors，可下载为 CSV 供业务修正后重新导入。</p></div><button type="button" onClick={downloadErrors}>下载错误文件</button><pre>{JSON.stringify(errors, null, 2)}</pre></section>}
    </div>
  );
}
