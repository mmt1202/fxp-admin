...
import { apiClient, type PaymentRecord, type ReconciliationRecord, type ReconciliationSummary } from '../api/client';
...
export function PaymentReconciliation() {
  ...
  useEffect(() => {
    // Data fetching is an external synchronization point for the current filters.
    ...
    <div className="reconciliation-page">
      ...
      <section className="table-section">
        ...
        <div className="table-scroll">
          <table>
            <thead><tr><th>本地订单号</th><th>第三方交易号</th><th>支付渠道</th><th>支付金额</th><th>本地状态</th><th>第三方状态</th><th>差异原因</th><th>结果</th><th>操作</th></tr></thead>
            <tbody>
              {records.map((record, index) => {
                const result = normalizeResult(record);
                const orderNo = getOrderNo(record);
                return (
                  <tr key={`${orderNo}-${getTransactionNo(record)}-${index}`}>
                    <td>{orderNo}</td><td>{getTransactionNo(record)}</td><td>{record.channel ?? '-'}</td><td>{formatMoney(record.amount ?? record.paidAmount)}</td><td>{record.localStatus ?? '-'}</td><td>{record.thirdPartyStatus ?? '-'}</td><td>{getDifferenceReason(record)}</td><td><span className={`result-badge ${result}`}>{resultLabel(result)}</span></td>
                    <td><button type="button" className="table-action" disabled={result !== 'exception' || repairingOrderNo === orderNo} onClick={() => handleRepair(record)}>{repairingOrderNo === orderNo ? '修复中' : '修复'}</button></td>
                  </tr>
                );
              })}
              {!loading && records.length === 0 && <tr><td colSpan={9} className="empty-cell">暂无对账结果</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
