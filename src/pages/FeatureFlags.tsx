...
export function FeatureFlags() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (selectedFlag) await configApi.updateFeatureFlag(selectedFlag.id, payload);
      else await configApi.createFeatureFlag(payload);
      await loadFlags();
  ...
  return (
    <div className="feature-flags-page">
      <div className="page-heading"><p className="eyebrow">Feature Flags</p><h1>功能开关配置中心</h1><p>集中管理 App 功能启停、灰度比例以及面向指定城市或用户的开放策略。</p></div>
      <div className="feature-flag-summary"><div><strong>{flags.length}</strong><span>开关总数</span></div><div><strong>{enabledCount}</strong><span>已启用</span></div><div><strong>{targetedCount}</strong><span>定向开放</span></div></div>
      ...
      <div className="feature-flag-layout">
        <section className="form-section">
          ...
          <div className="feature-flag-list">{flags.map((flag) => <article className={`feature-flag-row${String(flag.id) === String(selectedId) ? ' selected' : ''}`} key={flag.id}><button type="button" className="feature-flag-main" onClick={() => selectFlag(flag)}><strong>{flag.name}</strong><small>{flag.key} · 灰度 <span className="rollout-value">{flag.rolloutPercent}%</span></small><span className="feature-flag-targets">{flag.targetCities.length ? <span>城市：{flag.targetCities.join('、')}</span> : null}{flag.targetUsers.length ? <span>用户：{flag.targetUsers.join('、')}</span> : null}</span></button><span className={`feature-flag-status ${flag.enabled ? 'enabled' : 'disabled'}`}>{flag.enabled ? '已启用' : '已停用'}</span><button type="button" className="secondary-button" disabled={saveState === 'saving'} onClick={() => toggleFlag(flag)}>{flag.enabled ? '停用' : '启用'}</button></article>)}</div>
        </section>
        <form className="config-form" onSubmit={handleSubmit}>
          <section className="form-section">
            ...
            <div className="form-grid"><label className="field-label">开关 Key<input required value={form.key} onChange={(event) => updateField('key', event.target.value)} placeholder="例如 ai.review.v2" /></label><label className="field-label">开关名称<input required value={form.name} onChange={(event) => updateField('name', event.target.value)} placeholder="例如 AI 评房 V2" /></label><label className="field-label">灰度比例 <span className="rollout-value">{form.rolloutPercent}%</span><input type="range" min="0" max="100" value={form.rolloutPercent} onChange={(event) => updateField('rolloutPercent', Number(event.target.value))} /></label><label className="field-label">灰度比例数值<input type="number" min="0" max="100" value={form.rolloutPercent} onChange={(event) => updateField('rolloutPercent', Number(event.target.value))} /></label></div>
