...
export function SupportTickets() {
  ...
  useEffect(() => { if (!selectedId) return; let active = true; queueMicrotask(() => { if (active) setDetailLoading(true); }); apiClient.getSupportTicket(selectedId).then((ticket) => { if (active) { setDetail(ticket); setAssigneeId(ticket.assigneeId ? String(ticket.assigneeId) : ''); } }).catch((requestError: unknown) => { if (active) setError(requestError instanceof Error ? requestError.message : '工单详情加载失败'); }).finally(() => { if (active) setDetailLoading(false); }); return () => { active = false; }; }, [selectedId]);
  const selectedTicket = useMemo(() => (detail?.id === selectedId ? detail : tickets.find((ticket) => ticket.id === selectedId)), [detail, selectedId, tickets]);
