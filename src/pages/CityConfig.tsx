...
import { CityConfig as CityConfigType, CityConfigPayload, defaultCityConfigPayload, geoApi } from '../api/geo';

type SaveState = 'idle' | 'saving' | 'saved' | 'error';

const toFormPayload = (city?: CityConfigType): CityConfigPayload => city
  ? {
    name: city.name,
    code: city.code,
...
export function CityConfig() {
  ...
  const selectedCity = useMemo(() => cities.find((city) => String(city.id) === String(selectedId)), [cities, selectedId]);
  ...
  useEffect(() => {
    void Promise.resolve().then(loadCities);
  }, []);
  ...
  return (
    <div className="city-config-page">
      ...
      <div className="city-summary-grid">
        ...
        <div><strong>{cities.filter((city) => city.hot).length}</strong><span>热门城市</span></div>
      </div>
      ...
      <div className="city-config-layout">
        <section className="city-list-card">
          ...
          {loading ? <p className="muted-text">正在加载城市...</p> : null}
          {!loading && cities.length === 0 ? <p className="muted-text">暂无城市配置，请新增。</p> : null}
