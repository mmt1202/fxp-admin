...
type ModulePageProps = {
  title: string;
  description: string;
  module: Exclude<AdminModule['module'], 'feedback' | 'config'>;
};
...
const moduleLoaders: Record<Exclude<AdminModule['module'], 'feedback' | 'config'>, () => Promise<unknown>> = {
  dashboard: async () => {
...
export function ModulePage({ title, description, module }: ModulePageProps) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => setState({ loading: true }));
    loader()
      .then((data) => {
        if (!ignore) {
