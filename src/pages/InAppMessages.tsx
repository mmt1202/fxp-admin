...
import { InAppMessage, InAppMessagePayload, InAppMessageTargetType, InAppMessageType, messageStatusLabels, messagesApi, messageTypeLabels, targetTypeLabels } from '../api/messages';
...
export function InAppMessages() {
  ...
  return <div className="messages-page"><div className="page-heading"><p className="eyebrow">In-app Messages</p><h1>站内信管理</h1><p>创建系统公告、个人通知、活动通知、审核通知和会员通知，选择目标用户，预览内容并发送到 App 站内信。</p></div>{notice && <div className="notice-banner">{notice}</div>}<div className="messages-stats"><div><strong>{stats.total}</strong><span>消息总数</span></div><div><strong>{stats.sent}</strong><span>已发送</span></div><div><strong>{stats.delivered}</strong><span>触达用户</span></div><div><strong>{stats.readRate}</strong><span>累计已读</span></div></div><div className="messages-layout"><section className="form-section"><h2>创建消息</h2><form className="message-form" onSubmit={handleCreate}><label>消息类型<select value={form.type} onChange={(event) => setForm((current) => ({ ...current, type: event.target.value as InAppMessageType }))}>{Object.entries(messageTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>标题<input required value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="请输入站内信标题" /></label><label>正文<textarea required rows={6} value={form.content} onChange={(event) => setForm((current) => ({ ...current, content: event.target.value }))} placeholder="请输入 App 用户将看到的消息内容" /></label><label>目标用户<select value={form.targetType} onChange={(event) => setForm((current) => ({ ...current, targetType: event.target.value as InAppMessageTargetType }))}>{Object.entries(targetTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>{form.targetType === 'users' && <label>用户 ID<textarea rows={3} value={userIdsText} onChange={(event) => setUserIdsText(event.target.value)} placeholder="支持逗号、空格或换行分隔，例如 user_1001,user_1002" /></label>}{form.targetType === 'segment' && <label>用户分群<input value={form.targetSegment} onChange={(event) => setForm((current) => ({ ...current, targetSegment: event.target.value }))} placeholder="例如 membership_expiring_3d" /></label>}<label>跳转链接<input value={form.linkUrl} onChange={(event) => setForm((current) => ({ ...current, linkUrl: event.target.value }))} placeholder="可选，例如 /membership" /></label><label>定时发送<input type="datetime-local" value={form.sendAt} onChange={(event) => setForm((current) => ({ ...current, sendAt: event.target.value }))} /></label><div className="message-preview"><span>预览</span><h3>{form.title || '站内信标题'}</h3><p>{form.content || '站内信正文将在这里预览。'}</p><small>{messageTypeLabels[form.type]} · {previewTargets}</small></div><button type="submit" disabled={saving}>{saving ? '保存中...' : '创建消息'}</button></form></section><section className="form-section"><div className="message-list-header"><h2>消息列表</h2><select value={filterType} onChange={(event) => setFilterType(event.target.value as '' | InAppMessageType)}><option value="">全部类型</option>{Object.entries(messageTypeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>{loading && <div className="api-panel">正在加载站内信...</div>}<div className="message-list">{messages.map((item) => <button type="button" key={item.id} className={`message-list-item${item.id === selectedId ? ' active' : ''}`} onClick={() => setSelectedId(item.id)}><strong>{item.title}</strong><span>{messageTypeLabels[item.type]} · {messageStatusLabels[item.status]}</span><small>{formatDateTime(item.createdAt)}</small></button>)}</div></section><section className="form-section"><h2>预览与发送</h2>{selected ? <div className="message-detail"><span className={`message-status ${selected.status}`}>{messageStatusLabels[selected.status]}</span><h3>{selected.title}</h3><p>{selected.content}</p><dl><div><dt>类型</dt><dd>{messageTypeLabels[selected.type]}</dd></div><div><dt>目标</dt><dd>{targetTypeLabels[selected.targetType]}{selected.targetSegment ? ` · ${selected.targetSegment}` : ''}</dd></div><div><dt>发送人</dt><dd>{selected.senderName || '-'}</dd></div><div><dt>发送时间</dt><dd>{formatDateTime(selected.sentAt || selected.sendAt)}</dd></div><div><dt>触达/已读</dt><dd>{selected.deliveredCount ?? 0} / {selected.readCount ?? 0}</dd></div></dl><button type="button" disabled={saving || selected.status === 'sent'} onClick={() => handleSend(selected)}>{selected.status === 'sent' ? '已发送' : '确认发送'}</button></div> : <div className="api-panel">请选择消息查看预览。</div>}</section></div></div>;
}
