...
function normalizeCityConfig(city: Partial<CityConfig> & Record<string, unknown>): CityConfig {
  return {
...
function normalizeCityList(payload: unknown): CityConfig[] {
...
function normalizeCityResponse(payload: unknown): CityConfig {
  const body = unwrapPayload(payload);
