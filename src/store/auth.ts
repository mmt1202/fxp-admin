...
function isSuperAdmin(admin: AdminUser | null | undefined) {
  ...
  return roleText.includes('super') || roleText.includes('超级管理员') || admin.permissions.includes('*');
}

function withPermissionFallback(admin: AdminUser): AdminUser {
  if (!isSuperAdmin(admin) || admin.permissions.length > 0) return admin;
  return {
    ...admin,
    permissions: [...adminPermissions],
    role: { ...admin.role, permissions: [...adminPermissions] },
  };
}
...
function cacheAdmin(admin: AdminUser | null) {
  if (admin) window.localStorage.setItem(ADMIN_CACHE_KEY, JSON.stringify(withPermissionFallback(admin)));
...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  login: (token, admin) => {
    ...
    set({ token, currentAdmin, meLoading: false, meError: null });
  },
  logout: () => {
    tokenStorage.clear();
    cacheAdmin(null);
  ...
  fetchMe: async () => {
    ...
    try {
      ...
      const canUseDevFallback = import.meta.env.MODE === 'development' && cachedAdmin;
      set({
